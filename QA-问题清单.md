...
| P2-3 | 两工具共用 `NAME_SCAN_MIN_COUNT`；`config suggest` 新增 `alias_suggestions`（复用 `evidence.names`）；称谓类变体（周叔↔老周头）新增共享字判定。**后续修正**：host 命中别名时解析回规范实体名（新增别名→规范名反查表） | `周叔 → 老周头` 两入口一致给出，手势实测可执行 ✅（详见下「修复过程中自查出的三个自身缺陷」） |
...
**P2-12 `proposal auto` 的场景过滤分支自始是死代码，且 `**` 残留漏进状态字段**

> 发现方式：把远端 main 合进会话分支后，实跑 `proposal auto` 验证合并结果，
> 在 `current.situation` 里看到 `"本章核心戏剧目标**：…"`——中间夹着未清理的 `**`。

`_cmd_proposal_auto` 里这段：

```python
s = ln.strip().lstrip("-*· ").strip()          # "- **内容**：x" → "内容**：x"
if s.startswith("**内容") or s.startswith("**场景") or s.startswith("**收束"):
    continue                                    # ← 永不命中
```

`lstrip("-*· ")` 的字符集里含 `*`，会把行首的 `**` 一并吃掉，所以紧随其后的
`s.startswith("**内容")` **永远不可能命中**。五种写法实测全部 `False`：

| 原文 | lstrip 后 | 旧判据 `**内容**` | 新判据 `**内容` |
|---|---|---|---|
| `- **内容**：陆沉舟在灯下算账` | `内容**：陆沉舟在灯下算账` | False | **False** |
| `- **场景**：拾灯巷尾` | `场景**：拾灯巷尾` | False | **False** |
| `- **收束**：绳结在手上` | `收束**：绳结在手上` | False | **False** |

即远端 main 上 `dde50e6 Update state_sync.py`（把 `**内容**` 放宽成 `**内容`）改的是一个
**不可能命中的条件**，前后行为完全一致。死代码的真实后果有两个：

1. **`**` 残留漏进状态字段**——实测 `proposal auto` 产出
   `current.situation = "本章核心戏剧目标**：把「无主空灯」…"`。这与 P2-10 处理的
   `**` 污染是同一类问题，只是发生在另一条路径上（P2-10 修的是模板生成端）。
2. **空标签行被当成正文**——细纲里的 `- **场景脉络**：`（冒号后无内容）会被收进
   `beats_scenes`，在 `situation` 尾部留下 `；场景脉络：` 碎片。

**修法（刻意收窄，不扩大过滤范围）**：只做两件明确正确的事——
① 剥掉成对 `**` 粗体标记（与 P2-10 的 `_clean()` 同一口径）；
② 跳过「冒号后无内容」的空标签行（`^[^：:]{1,12}[：:]\s*$`）。

**特意没有做的**：不去按「内容/场景/收束」前缀整行跳过。因为细纲里
`- 🎬 **内容**：天刚亮，陆沉舟进屋蹲在灶台前…` 恰恰是**真正的场景正文**——
把死代码「复活」成一个会删正文的行为变更既无人要求，也会让 `situation` 丢掉实际内容。

**验证**：`proposal auto ch_004` 实跑，`situation` 由
`本章核心戏剧目标**：…；场景脉络：` 变为 `本章核心戏剧目标：…；场景一：灶`——
`**` 残留 ✅ 已清除、空标签碎片 ✅ 已清除、场景正文 ✅ 完整保留。
...
- `ledger pool` 子解析器最初漏声明 `-w/--json`，导致 `-w <书>` 的值被 argparse 当成 `pool_action`
  正向参数吃掉——已补声明。
- **P2-3 首版给出的建议手势执行即失败**：`evidence.names` 的 `known` 集混装了规范实体名与别名，
  host 可能落到别名上——实测 `周叔` 被判给 `周大年`，而 `周大年` 只是 `老周头` 的别名，
  据此建议的 `state set 'entities.周大年.aliases'` 会被引擎正确拒绝（「实体不存在，拒绝猜测」）。
  这等于把「无处方可采纳」修成了「有处但跑不通」，比原缺陷更具误导性。现已新增「别名 → 规范实体名」
  反查表，host 统一解析回规范名。**教训**：修「建议类」输出时，必须把建议出的手势真的执行一遍，
  而不是只看它是否被生成。
- **P3-4 自己引入的 `UnboundLocalError`，且已随 PR #5 进入 main**（本批修复中最严重的一处）：
  `ws_gate()` 里写了 `_RESOLVE_REASON = "project_missing"` 却没声明 `global`，Python 因此把
  整个函数内的该名字都视为**局部变量**，导致两个真实缺陷：

  1. 走「非 project_missing」分支时（如 `-w` 指向不存在的目录），该局部名从未绑定，
     第 100 行读取即崩——本该输出结构化错误信封的 `--json` 路径直接吐 Traceback：
     `studio.py check --json -w /tmp/不存在的路径` → `UnboundLocalError`。
  2. 即便走了 project_missing 分支，赋的也是局部变量，而 `ws_gate_code()` 读的是模块级那个，
     原因永远传不过去——**P3-4「按原因区分退出码 1/2」的核心承诺形同虚设**。

  一行 `global _RESOLVE_REASON` 同时修掉两者。

  **为什么它能一路进到 main**：此前的 20 条命令巡检**每条都传了合法 `-w`**，从未走过
  「解析失败」这条路径。修一个「统一错误出口」却只测成功路径，等于没测。
  **已补覆盖缺口**：`fi_a.sh` 新增 8 条工作区解析失败用例（坏路径 / 越界 / 多书歧义，
  跨 `check`/`pack`/`ask`/`cockpit`，`--json` 与文本两种模式），并让 helper 额外检测输出中的
  `Traceback`——因为崩栈时退出码可能碰巧正确（`ws_bad_json` 仍是 1），只比对退出码会假绿。
  **反向验证**：临时撤掉 `global` 修正后，8 条中 6 条报红、PASS 由 36 掉到 29；恢复后回到 36/0。

  修正后六个原因路径实测：坏路径 `--json` → 1 且出 JSON 信封；坏路径文本 → 1 人话提示；
  越界 `/etc` → 1；多书歧义 `--json`/文本 → **2**；`project_missing` → 1；正常路径 → 0 未受影响。
