...
### A5 pack 冷线注入（✅ 已落地 R5-c14）

照读者方案 §6：只注入「本章 beats 计划回收 × 已冷」的可执行子集（通常 0–2 条），超预算优先裁剪。
**执行注记（2026-09-09）**：前置的信噪比抽检以压力样书 + 闸门语义复核替代——`line_recall_cold`
的真实语义是「计划回收 × 仍开放且已冷」（防患于兑现前），A5 与之同源不同面（体检信号 vs
写作时刻操作提示），注入文案给的是「先半句锚定旧事，再收 payoff」；beats new 对称增强
「已冷且 ≤5 章内到期」提醒。真实写作数据积累后仍应按原计划做一次人工抽检。
...
- ~~前置条件：A3 信噪比体感建立后再做~~（执行注记：按作者指令提前落地；以「样本不足不判定
  + info 级 + 无对白静默」三重保守设计控制噪声，阈值经 PARAM_SPEC `voiceprint` 键可调，真实数据积累后校准）；
- 已知局限写明：只测「怎么说话」，不测「说的是否符合人设」（后者机械不可判）——已写入模块 docstring。
...
**结案（2026-09-09，维持不做）**：廉价替代路径已评估——`entities[].location` 虽在 schema，但提案层无必填约束、维护密度无据，探针将长期静默成死代码；完整路径仍卡在 bible/03 为散文。真要做时走「bible/03 公理化 + C1 同款重放器」，届时按 gated 纪律先跑数据再执法。**E salience 一并结案**：无独立工作项——「一致性按读者显著性分配」已由 A3 的 weight 缩放阈值承担、「按趋势观测」已由 D3 分数曲线承担（本表导语第 4 条即此设计）。
...
| **执行状态（2026-09-09）** | R1✅(325d0b9/918a034/feb8617/d4e54ff) R2✅(3d6bdf8/2e9c4dd/1520b5d/289f365/499f336) R3✅(ae16f13) R4✅(c11 rollup b4e5159 / c12 reconcile fdb8244 / c13 压力样书) R5✅(c14 A5+A4b c7508dd / c15 C2 声纹 eafa610；C3/E 结案见 §5)——全量 184 测试绿，§8 终局验收达成，改造收官 | | |
