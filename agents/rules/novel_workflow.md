...
不要另起一个空的 `## 任务书` 标题（与四节同级会变成空节）。pack 的 P0 把任务书四节整块投递给子代理。
...
- 退回边（主控验收，进 Stage 4 的闸门）：
  ...
  - 情节事实错、或「必须保留」在 raw 里就已破 → 按#拒收语义回 Stage 2，**不要**派 guard 救情节；
  - guard 退出码 2（停机报告情节硬伤）→ 视同拒收，回 Stage 2。
  重派与拒收共用上限，见#拒收语义。

### 拒收语义

拒收判定权全在主控。触发：Stage 2 中间验收判 raw 不可救（不派 guard）；或 Stage 3 验收判情节硬伤
...
（P0/P1 必读，P2 索引按需）、可写路径清单、退出码约定（0=交付；2=缺语境或情节硬伤停机——这是
...
drafter/guard 写 raw/final（#拒收语义里的例外代笔除外）。
