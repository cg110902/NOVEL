...
- **标准执行流程 (Actions)**：
  ...
  3. **前置事实与称谓精准锚定**：完成 `## 本章法定事实与称谓对校清单` 的填写，作为后续 Drafter/Editor/Auditor 的唯一对校基准；
...
- **派发时序（每章 3 次标准派发 + 按需手术刀修复）**：
  ...
  3. Editor 回执唤醒主控 → **在单次 `invoke_subagent` 调用中同时并发派发 Reader、Critic 与 Auditor**（原生三轨并发质检）：
  ```json
  {
    "Subagents": [
...
- **Auditor 双轨硬矛盾分流（定向手术刀修复 Surgical Patch）**：
  - 若 Auditor 报告包含 🔴 **确凿硬矛盾**（如已死复活、闭门瞬移、充能透支、**称谓擅自漂移、前情修饰词拔高**）：主控**绝不全章重写**，立即向 Editor 下发**定向手术刀修复令**（附带行号与修复建议）；Editor 仅替换冲突句子后覆盖落盘；
