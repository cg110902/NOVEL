...
- **取证工具（只读取证三件套 · 注意力预算纪律）**：
	...
	5. `python studio.py simulate impact/branch`：**剧情推演沙盒**——重大剧情转折前用 `simulate impact` 测算因果链波及，遇创作瓶颈用 `simulate branch` 生成隔离假说参谋单；
...
- **标准执行流程 (Actions)**：
  ...
  2. **吸纳催更便签 4 大核心情报（精准制导细纲；情报源 = cockpit 催更雷达，勿重复翻读原文）**：
   ...
   4. **10 大叙事章型 (form) 动态轮换**：
      - 丰富使用 10 大章型（生死博弈/战后清点/危机逼近/暗流汇聚/信息错位/幕后布局/同门扬威/烟火微澜/探秘开箱/心理对弈），严禁单一章型占比超 40%；若连续章节使用相同 `form`，必须在 front-matter 补充 `form_reason`；
   5. **因果依赖与视界对齐**：关注脚手架中的硬提醒，若提示前置因果未达成，严禁强行回收该线索，应先安排前置动作；专注当卷里程碑，无需分神其他分卷细节；
   6. **注入读者高光期待与避坑**：对照便签中的【最想看】设计爽点释放，坚决避开便签中的【最怕踩】毒点雷区；
   7. **🎯 场景脉络三大动力引擎（严格 2 个核心大场景，最多 3 个）**：
      - **核心戏剧支点 (Pivot)**：明确人物核心诉求、阻碍或反常识错位；
      - **感官物象锚点 (Anchor)**：选定具象物理物件承载动作与生活质感；
      - **物理因果气口 (Bridge)**：前一幕的动作余波直接引出下一幕物理起因，坚决杜绝 PPT 赶场式生硬跳切。
   8. **👁️ POV 视角设计（单主视点为主，双线对峙为辅）**：
      - 默认以主角视角沉浸推进；遇决战蓄势、反派密谋或震撼显圣时，可灵活设计双线视角，拉大信息差反差爽感；并在细纲中清晰标注视点人。
   9. **创意灵魂重塑与落盘**：主控行使最终裁决权，将独家设计的反套路笑点、戏剧冲突与反转底牌写入当章 `workspace/<书名>/outlines/vol_XX/beats/ch_XXX.md`。
