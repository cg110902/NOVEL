...
- 规划各卷的核心矛盾演进、关键转折点与高潮规划。

### 2. 细纲装配（Stage 1 · 适配 Gemini 的关键输入）
在 `outlines/vol_XX/beats/ch_XXX.md` 中为 Gemini 提供清晰有力的输入：
- **文风基准**：通俗直白大白话，禁止冷峻阴暗描写；
- **明确不可调和的利益冲突**：明确双方无法退让的诉求，防止 Gemini 自动温和化；
- **场景脉络与动作焦点**：给出场景发展与关键动作走向，避免说明性自问自答；
- **伏笔与线索动作**：明确当章涉及的 `GUN-*` / `KNO-*` / `MIS-*` 动作；
- **物理收尾锚点**：预定章末卡在哪个动作或危机瞬间。
