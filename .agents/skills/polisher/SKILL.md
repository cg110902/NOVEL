...
> 派发令已给定工作区与章节。**开工首步必须调用 `view_file` 同时读取本手册与重塑稿 `manuscript/vol_XX/raw/ch_XXX_v2.md`！任务途中绝对严禁中途回读倒嚼本手册！严禁调用 `list_dir` / `find_by_name` 搜寻目录！**
...
- 📖 **准读文件**：`.agents/skills/polisher/SKILL.md`（首步锁定规范）与 `manuscript/vol_XX/raw/ch_XXX_v2.md`（单次全读，禁切片）；
