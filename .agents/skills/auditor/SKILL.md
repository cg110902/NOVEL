...
3. **步骤 3【顺手预制修补配方并交付回执 · 省时接力】**：
   - 查看 `log/audit/ch_XXX.md`；
   - 若步骤 2 发现了语义出戏/常识硬伤：调用 `replace_file_content` 将条目补充在 `## 🧠 语义逻辑与出戏审查` 下，并将 front-matter 的 `logic: 0` 改为实际问题数；
   - ⚡ **【预制修补配方（省时核心）】**：在报告中顺手写出精确的替换块：
     ```text
     - 行号：第 N 行
     - TargetContent: "正文原句"
     - ReplacementContent: "修改后的通俗正文原句"
     - 理由: "一句话说明原因"
     ```
     让 Fixer 拿到报告后无需重新思考或搜寻，直接照方抓药秒级替换！
   - 若无额外语义问题：保持原报告不变；
