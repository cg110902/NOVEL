...
1. **时空与在场角色（`current`）——章末现场快照，12 字段刷新义务**：
   ...
   - 🔄 **以下字段凡本章发生变动必刷，最常漏刷**：
     - `power_level`：境界/位阶/职级突破或变化（按题材载体的措辞：引气入体、晋升警督、升舱等）——**本章的核心突破事件尤其不能漏**；
     - `loadout`：主角当前常驻手段体系（按题材自定：功法身法 / 专业技能与人脉 / 装备与权限，连同招牌手法与保命底牌）；
     - `abilities`：新习得技能/本领/能力；
...
> ⚠️ 下文示例中的题材名物（功法/宗门/丹药等）**仅为格式演示**，请一律替换为本书题材的实际实体与措辞——机制与字段与题材无关。
