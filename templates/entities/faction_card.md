...
<!-- 💡【Stage 0 架构师指南（填写后可删）】
     ...
     ★ 状态同步提醒：本 YAML 中的 id, name, type, scale_tier, tier_name, attitude, core_assets, diplomacy, status 属于 state/entities.json 法定白名单；leader 与 headquarters 属于卡片语义扩展，入库 entities.json 时切勿带入未知键。
     ★ 全题材适配：
