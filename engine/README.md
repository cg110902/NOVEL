...
| `changelog.py` | 事件溯源层：`state/changelog.jsonl` 字段级变更事件流（save_state 唯一写入咽喉自动派生；外部改动 load 时自动补录；快照回滚不清空历史而是记为事件）；`fold(base, events) == 磁盘` 核心不变量供 verify 对账，为 `state at`（B2）与 `blame`（B3）供底 | 标准库（追加式 JSONL + 规范哈希） |
