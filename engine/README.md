...
| checks.py | check：结构/schema/算术/逾期/form 占比 + 上章对照与自交检报数（style_notes_copy/words_band_crowded/acceptance_empty_criterion/line_action_*/retired_entity_on_stage）；sync 前置 `review_gate`（Stage 5 校对注记验收覆盖）；review 注记骨架数据（`review_skeleton`）、提案三方事实对照（`proposal_cross_facts`，含知识线揭示时机对照） |  
