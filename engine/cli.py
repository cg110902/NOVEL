...
COMMAND_HELP = {
    ...
    "check": "结构/schema/算术体检（errors 只允许事实级；有 errors 退出码 1；新书 Stage 0 待办不阻断；--trend 分数曲线；--bisect 快照二分定位不变量首次破坏；--full 全量；--accept 确认消音）",
...
def _build_subparsers(sub: argparse._SubParsersAction) -> None:
    ...
    q.add_argument("--full", action="store_true",
                   help="全量输出：关闭历史折叠与确认基线过滤，显示所有原始 findings")
    q.add_argument("--accept", default=None,
                   help="确认消音：fp 或前缀（逗号分隔，可多个），仅 warnings/infos 可确认，errors 拒绝")
    q.add_argument("--accepted", action="store_true", help="列出确认基线（已确认消音的 findings）")
    q.add_argument("--unaccept", default=None, help="撤销确认：fp 或前缀（逗号分隔，可多个）")
    ...
    q.add_argument("--full", action="store_true", help="同 check --full")
    q.add_argument("--accept", default=None, help="同 check --accept")
    q.add_argument("--accepted", action="store_true", help="同 check --accepted")
    q.add_argument("--unaccept", default=None, help="同 check --unaccept")
    q.set_defaults(func=cmd_check)
    ...
    r.add_argument("--json", action="store_true", default=argparse.SUPPRESS)
    r.set_defaults(func=cmd_proposal)
    r = pp.add_parser("patch", help="对在途提案快速打补丁（免手改 JSON）")
    r.add_argument("chapter")
    r.add_argument("-w", "--workspace", default=argparse.SUPPRESS)
    r.add_argument("--location", help="修改当前地点")
    r.add_argument("--time", help="修改当前时间")
    r.add_argument("--injury", help="修改伤势描述")
    r.add_argument("--power", help="修改境界描述")
    r.add_argument("--assets", help="修改随身资产描述")
    r.add_argument("--add-char", help="添加在场人物")
    r.add_argument("--remove-char", help="移除在场人物")
    r.add_argument("--title", help="修改章标题")
    r.add_argument("--synopsis", help="修改梗概")
    r.add_argument("--json", action="store_true", default=argparse.SUPPRESS)
    r.set_defaults(func=cmd_proposal)
