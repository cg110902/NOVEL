...
流水线按**双层极速高精架构**执行，各子代理独立读取自身 `.agents/skills/<角色>/SKILL.md` 开展作业，禁止串岗：
...
| **精修师 Editor**<br/>(Stage 3) | `inherit` | `raw/ch_XXX_v3.md` | `raw/ch_XXX_v3.md`<br/>(`replace_file_content`) | `Copy-Item` (唯二操作) | **双核精修与脱水**：Copy-Item 复制 v1 为 v3，单次全读后倒序对 **4~6 处核心大块** 手术刀替换（通俗脱水、去冷脸面瘫词、手机短句），绝对零命令，落盘即走。 |
...
1. **主控下达 · 标准 4 行工序派发令（严格闭合，零主观说教）**：
   ...
   - 执行指令：起手直接执行[动作] ➔ 准写=[调用工具直接物理落盘（禁传ArtifactMetadata）] ➔ 执行[专属验证命令] ➔ 3行回执交卷（禁倒嚼/禁发正文聊天/禁写脚本/落盘即走）
   ```
...
5. **专职边界与免多余回读铁律**：
   ...
   - **Editor**：单次全读 `raw_v3` 后，聚焦 **4~6 处核心大块** 自下而上倒序执行 `replace_file_content`，严禁全文推倒重写；
