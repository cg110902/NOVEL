...
0. **开工第一反射动作（Cockpit First Reflex ）**：
   - 只要主控收到人类指令（如“继续下一章”、“推进剧情”或“写第X章”），**第 1 个 Tool Call 必须 `run_command` 执行 `python studio.py cockpit --json`**；
...
## 三、 五大创作不变量（50W+ 超长篇一致性底座）

1. **事实一致不吃书**：正文定稿事实为源，`state/*.json` 为确定性引擎真值底座，确保超长篇连载事实逻辑不坍塌。
2. **伏笔暗线有因果拓扑**：重要伏笔（`GUN-*`）、秘密（`KNO-*`）、认知差/误会（`MIS-*`）登记入 `state/lines.json`，支持 `requires` 前置依赖 DAG，引擎自动阻断无序跳步。
3. **人物登场皆有据**：核心常驻人物建立人物卡（`characters/*.md`），登场核心实体登记入 `state/entities.json`。
4. **分卷视界隔离（Volume Locality）**：50W+ 长篇自动按分卷（`vol_XX`）生命周期隔离，非当卷实体自动休眠冷备，确保单章上下文始终清爽，杜绝注意力稀释。
5. **动态张力与物象锚定（AI 专属拓扑底座）**：通过两两恩怨张力（`relations`）与高光情境切片（`golden_quote`），赋能起草员写透对手戏机锋与跨卷细节呼应。所有拓扑与图表专供 AI 消费，人类作者仅终审定稿，以“继续下一章”全自动推进。
6. **戏剧动力学与自愈处方驾驶舱（AI 定制黑盒）**：引擎实时计算开篇余震（`aftershock`）、悬顶危机倒计时（`active_pressures`）、现场信息差机锋（`dramatic_irony`）并注入 P0 上下文；通过 `studio.py cockpit` 统一提供主控态势驾驶舱与机器自愈处方（Remedies），主控遇错按方自愈，只有系统死锁才向人类求助。
...
4. 🚫 **Critic 定位为催更便签**：
   - 催更便签供主控下章细纲参考，不具备阻断当章流水线的权力；流水线默认直通 Stage 5。
5. 🛡️ **引擎黑盒交互原则**：
   - 严禁任何代理(agent)尝试读取或修改 `engine/*.py` 源码！所有操作一律通过 `python studio.py <命令>` 交互。
