...
|---|---|---|---|
| **主控 (Director)** | 宿主主代理 | Stage 0 / 1 / 4 | 全局导演：负责世界观大纲设定、防雷同单章细纲装配、调度子代理、依据 Guard 500字简报极速状态同步与快照封存。 |
| **起草员 (Drafter)** | 原生子代理 (Subagent) | Stage 2 | 情节起草：彻底松绑放飞算力，吸纳上章 500~800 字完整尾声场面，全力将细纲转化为冲突饱满、人物生动的毛坯初稿 `raw/`。 |
| **精修师 (Guard)** | 原生子代理 (Subagent) | Stage 3 | 商业重铸：依据 `craft_guard.md` 担任金牌总编，深度重写填坑，产出丝滑定稿 `final/`，交付 500 字结构化事实简报。 |
...
4. **Stage 3 商业重铸（Guard）**：主控派发子代理（遵循 `craft_guard.md` 深度重写），生成定稿 `manuscript/vol_XX/final/ch_XXX.md` 并提交 500 字结构化事实简报。
...
2. **伏笔暗线有台账**：重要伏笔（`GUN-*`）、秘密（`KNO-*`）登记入 `state/lines.json`。
...
2. **主控零盲读原则**：主控严禁在主会话中对 `raw/` 和 `final/` 执行全文 `view_file`，仅依据 Guard 的 500 字结构化事实简报进行状态同步。
