...
1. **原生多智能体流转合并（一步到位全自动流水线）**：
   - **Director（主控 · Stage 0 / 1 / 5）**：全局统筹世界观与分卷主线；端到端自动化连续调度 Drafter → Editor → Reader/Critic；Stage 1 主动吸纳上章催更便签；审定提案并一键执行 `sync` 闭环封存状态机。
   - **Drafter（起草子代理 · Stage 2）**：彻底松绑想象力与算力，承接上章情境与细纲，篇幅在 2000~3000+ 汉字自由舒展，以鲜活对白与微波澜小拉扯起草高能毛坯初稿 `raw/`，落盘即闭环。
   - **Editor（精修重塑子代理 · Stage 3）**：金牌总编重铸定稿，以连贯丝滑欲罢不能为唯一指标，砍掉 80% 无效景物，动态调节对白占比，产出纯净定稿 `final/`，落盘即闭环。
   - **Reader（事实审计子代理 · Stage 4 轨A）**：客观提炼 4 大核心事实（在场状态、重要实体、主线伏笔、大额收支），直接装配标准增量提案 JSON (`state/inbox/`)，落盘即闭环。
   - **Critic（老白催更便签 · Stage 4 轨B）**：扮演十年老白追更书虫，与 Reader **并发运行（零时延重叠）**，产出 150~300 字催更便签 `log/critic/` 专供下一章细纲构思参考（**当章不设自动打回，流水线直通 Stage 5**）。
